...
def get_args():
    ...
    parser.add_argument('--model_save_path', type=str, default='save_model/thucnew_bert.bin', help='save path for model')
    parser.add_argument('--data_path', type=str, default='./data/thucnews', help='save path for model')
    parser.add_argument('--label_txt', type=str, default='./data/thucnews/label.txt', help='label txt file')
...
def collate_fn(batch, tokenizer):
    input = tokenizer([i['text'] for i in batch], padding=True, truncation=True, max_length=128, return_tensors='pt')
    input['labels'] = torch.LongTensor([i['label'] for i in batch])

    return input

def collate_test_fn(batch, tokenizer):
    input = tokenizer(batch, padding=True, truncation=True, max_length=128, return_tensors='pt')
...
def gen_dataloader(data_path,tokenizer,batch_size=32):
    ...
    train_dataset = SequenceClassificationDataset(os.path.join(data_path,'train.txt'))
    test_dataset = SequenceClassificationDataset(os.path.join(data_path,'dev.txt'))
...
def main():
    ...
    if not args.do_predict:
        ...
        model=BertForSequenceClassification.from_pretrained(args.pretrained_path,config=config)
        model.to(device)
