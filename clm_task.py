...
def get_args():
    ...
    parser.add_argument('--pretrained_path', type=str, default='pretrained_model/gpt2-chinese', help='choose a model_path')
...
def gen_dataloader(data_path,tokenizer,batch_size=32):
    ...
    train_dataset = LanguageModelDataset(os.path.join(data_path,'train.txt'))
    test_dataset = LanguageModelDataset(os.path.join(data_path,'dev.txt'))
...
def evaluate(model, data_loader, device='cpu'):
    ...
    labels = labels_all[:, 1:].reshape(-1)
    preds = predict_all[:, :-1].reshape(-1)
...
def main():
    ...
    if not args.do_predict:
        ...
        model=AutoModelForCausalLM.from_pretrained(args.pretrained_path,config=config)
        model.to(device)
